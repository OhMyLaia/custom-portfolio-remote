    ...
    <form class="flex flex-col gap-5" @submit.prevent="onSubmit">
      <div class="flex flex-col gap-1.5">
        ...
        <input
          ...
          type="text"
          required
          class="bg-surface text-foreground placeholder-foreground/40 border border-foreground/20 px-4 py-2.5 rounded-full transition-colors focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent"
        />
      </div>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          rows="5"
          required
          class="bg-surface text-foreground placeholder-foreground/40 border border-foreground/20 px-4 py-3 rounded-2xl transition-colors focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent resize-none"
        />
      </div>
      ...
      <div class="flex items-center justify-between gap-4">
        <button
          ...
          class="group inline-flex items-center gap-2 bg-accent text-black px-5 py-2.5 rounded-full text-sm font-semibold uppercase tracking-wider transition-all hover:opacity-90 hover:pl-6 disabled:opacity-50 disabled:hover:pl-5"
        >
          {{ status === 'sending' ? t('contact.sending') : t('contact.send') }}
          <span aria-hidden="true" class="transition-transform group-hover:translate-x-0.5">&rarr;</span>
        </button>

        <div v-if="profile?.social?.linkedin || profile?.social?.github" class="flex items-center gap-4">
          <span class="text-xs font-semibold uppercase tracking-wider text-foreground/50">{{ t('contact.findMe') }}</span>
          <a
